...
The root `AGENTS.md` is the repository's normative control plane. Its rules apply before deeper lookup. QMD is an upstream-derived local Markdown search CLI maintained as a Hraness fork while retaining the public `@tobilu/qmd` identity and upstream operating model.
